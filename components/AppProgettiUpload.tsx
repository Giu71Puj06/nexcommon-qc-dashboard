...
function getElaboratoForPdfCell(row: any) {
  const items = getElaboratiForExportRow(row)
  ...
  // Rilievo con piu' codici elaborato: i codici vanno impilati nella stessa cella,
  // non piu' spalmati su piu' righe della tabella.
  if (items.length > 1) {
    return items.join("\n");
  }
...
function prepareRowsForPdfExport(rows: any[]) {
  // UNA RIGA PER RILIEVO, come in dashboard.
  // Un rilievo che cita piu' codici elaborato NON viene piu' duplicato in una riga
  // per codice: i codici restano nella stessa riga, impilati nella cella "Elaborato"
  // (vedi getElaboratoForPdfCell). Cosi' il numero di righe del PDF coincide con il
  // numero di rilievi mostrato dalla dashboard per quella disciplina.
  return [...(rows || [])];
}
...
// ANAGRAFICA FIRME MODIFICABILE
// SIGNATURE_DATABASE resta l'anagrafica di base, immutata.
// A questa si sommano i nominativi aggiunti dall'utente e si sottraggono quelli rimossi:
// entrambe le cose vivono nel browser (localStorage), per postazione.
const CUSTOM_SIGNATURES_STORAGE_KEY = "nexcommon_firme_personalizzate";
const HIDDEN_SIGNATURES_STORAGE_KEY = "nexcommon_firme_rimosse";

let CUSTOM_SIGNATURES: SignatureDatabaseEntry[] = [];
let HIDDEN_SIGNATURE_KEYS: string[] = [];

function signatureKey(entry: SignatureDatabaseEntry) {
  return `${entry.role}||${entry.name}||${entry.discipline}`;
}

function setSignatureRegistry(custom: SignatureDatabaseEntry[], hidden: string[]) {
  CUSTOM_SIGNATURES = Array.isArray(custom) ? custom : [];
  HIDDEN_SIGNATURE_KEYS = Array.isArray(hidden) ? hidden : [];
}

function getBaseSignatureEntries(): SignatureDatabaseEntry[] {
  return (SIGNATURE_DATABASE as readonly any[]).map((entry) => ({ ...entry })) as SignatureDatabaseEntry[];
}

function getAllSignatureEntries(): SignatureDatabaseEntry[] {
  return [...getBaseSignatureEntries(), ...CUSTOM_SIGNATURES].filter(
    (entry) => !HIDDEN_SIGNATURE_KEYS.includes(signatureKey(entry))
  );
}

function isCustomSignatureEntry(entry: SignatureDatabaseEntry) {
  const key = signatureKey(entry);
  return CUSTOM_SIGNATURES.some((item) => signatureKey(item) === key);
}

function getSignatureEntryByKey(key = "") {
  return getAllSignatureEntries().find((entry) => signatureKey(entry) === key);
}

function getResponsabileOptions() {
  return getAllSignatureEntries().filter(
    (entry) => entry.role === "Responsabile Tecnico" || entry.role === "Coordinamento"
  );
}
...
function getIspettoreOptions() {
  ...
  return getAllSignatureEntries().filter(
    (entry) =>
      entry.role === "Ispettore Tecnico" &&
      // I nominativi aggiunti a mano sono sempre selezionabili:
      // l'elenco chiuso vale solo per l'anagrafica di base.
      (allowedInspectors.has(entry.name.trim()) || isCustomSignatureEntry(entry))
  );
}
...
function exportDetailPdf(rows: any[], title = "", headerData: PdfHeaderData = {}) {
  ...
  if (templateMode) {
    ...
    // Nessun filtro sul tipo: la scheda riporta le stesse righe che la dashboard
    // conta per la disciplina (NC, OSS, Da NC a OSS e Nessun rilievo), una per rilievo.
    const printableRows = prepareRowsForPdfExport(rows);
  ...
  const tableRows = rows.map((r: any, i: number) => {
    ...
    return [
      ...
      // Gia' ripulito codice per codice: qui NON si applica cleanPdfText,
      // altrimenti gli a capo fra piu' codici elaborato verrebbero schiacciati.
      getElaboratoForPdfCell(r),
...
function isKnownInspectorAuthor(author: any) {
  ...
  return getAllSignatureEntries().some((entry) => {
...
function findSignatureEntryByPersonName(name: string) {
  ...
  return (
    getAllSignatureEntries().find((entry) => {
      const entryName = normalizeCommentAuthor(entry.name || "").toLowerCase();
...
function DetailPanel({ rows, title, onReset, sourceFiles = [] }: any) {
  ...
  // Anagrafica firme: aggiunte e rimozioni salvate nel browser.
  const [customSignatures, setCustomSignatures] = useState<SignatureDatabaseEntry[]>([]);
  const [hiddenSignatureKeys, setHiddenSignatureKeys] = useState<string[]>([]);
  const [showAnagraficaFirme, setShowAnagraficaFirme] = useState(false);
  const [nuovoNominativo, setNuovoNominativo] = useState({
    name: "",
    discipline: "",
    role: "Ispettore Tecnico",
    signature: "",
  });

  React.useEffect(() => {
    try {
      const custom = JSON.parse(localStorage.getItem(CUSTOM_SIGNATURES_STORAGE_KEY) || "[]");
      const hidden = JSON.parse(localStorage.getItem(HIDDEN_SIGNATURES_STORAGE_KEY) || "[]");
      const customList = Array.isArray(custom) ? custom : [];
      const hiddenList = Array.isArray(hidden) ? hidden : [];

      setCustomSignatures(customList);
      setHiddenSignatureKeys(hiddenList);
      setSignatureRegistry(customList, hiddenList);
    } catch {
      setSignatureRegistry([], []);
    }
  }, []);

  // Le funzioni di anagrafica non sono dentro React: le teniamo allineate allo stato.
  setSignatureRegistry(customSignatures, hiddenSignatureKeys);

  function persistSignatureRegistry(custom: SignatureDatabaseEntry[], hidden: string[]) {
    setCustomSignatures(custom);
    setHiddenSignatureKeys(hidden);
    setSignatureRegistry(custom, hidden);

    try {
      localStorage.setItem(CUSTOM_SIGNATURES_STORAGE_KEY, JSON.stringify(custom));
      localStorage.setItem(HIDDEN_SIGNATURES_STORAGE_KEY, JSON.stringify(hidden));
    } catch {
      alert("Non è stato possibile salvare l'anagrafica firme in questo browser.");
    }
  }

  function aggiungiNominativo() {
    const name = String(nuovoNominativo.name || "").trim();
    const discipline = String(nuovoNominativo.discipline || "").trim();
    const role = String(nuovoNominativo.role || "").trim();

    if (!name) {
      alert("Indicare il nominativo, per esteso (es. Ing. Mario Rossi)");
      return;
    }

    if (!nuovoNominativo.signature) {
      alert("Caricare la firma in formato .png, .jpg o .jpeg");
      return;
    }

    const entry: SignatureDatabaseEntry = { name, discipline, role, signature: nuovoNominativo.signature };
    const key = signatureKey(entry);

    if (getAllSignatureEntries().some((item) => signatureKey(item) === key)) {
      alert("Esiste già un nominativo con lo stesso nome, disciplina e ruolo.");
      return;
    }

    persistSignatureRegistry(
      [...customSignatures, entry],
      hiddenSignatureKeys.filter((item) => item !== key)
    );

    setNuovoNominativo({ name: "", discipline: "", role: "Ispettore Tecnico", signature: "" });
  }

  function rimuoviNominativo(entry: SignatureDatabaseEntry) {
    const key = signatureKey(entry);

    if (!confirm(`Rimuovere ${entry.name} (${entry.role}) dall'elenco?`)) return;

    const custom = customSignatures.filter((item) => signatureKey(item) !== key);
    const hidden = Array.from(new Set([...hiddenSignatureKeys, key]));

    persistSignatureRegistry(custom, hidden);

    // Se il nominativo rimosso era selezionato, si toglie anche dalla testata.
    setPdfHeader((prev) => {
      const nextKeys = (prev.ispettoreKeys || []).filter((item) => item !== key);
      const entries = nextKeys
        .map((item) => getSignatureEntryByKey(item))
        .filter(Boolean) as SignatureDatabaseEntry[];
      const responsabileRimosso = prev.responsabileTecnicoKey === key;

      return {
        ...prev,
        ispettoreKeys: nextKeys,
        ispettore: entries.map((item) => item.name).join(" / "),
        firmaIspettoreImages: entries.map((item) => item.signature).filter(Boolean),
        responsabileTecnicoKey: responsabileRimosso ? "" : prev.responsabileTecnicoKey,
        responsabileTecnico: responsabileRimosso ? "" : prev.responsabileTecnico,
        firmaResponsabileImage: responsabileRimosso ? "" : prev.firmaResponsabileImage,
      };
    });
  }

  function ripristinaAnagraficaFirme() {
    if (!confirm("Ripristinare l'elenco originale, annullando aggiunte e rimozioni?")) return;
    persistSignatureRegistry([], []);
  }

  function caricaFirmaNuovoNominativo(file?: File) {
    if (!file) {
      setNuovoNominativo((prev) => ({ ...prev, signature: "" }));
      return;
    }
    ...
    reader.onload = () => {
      setNuovoNominativo((prev) => ({
        ...prev,
        signature: typeof reader.result === "string" ? reader.result : "",
      }));
    };
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              <label style={{ ...labelStyle, marginBottom: 0 }}>Anagrafica nominativi e firme</label>
              <button
                type="button"
                onClick={() => setShowAnagraficaFirme((prev) => !prev)}
                style={{
                  border: "1px solid #cbd5e1",
                  background: showAnagraficaFirme ? "#e0f2fe" : "white",
                  borderRadius: 8,
                  padding: "4px 10px",
                  fontSize: 12,
                  cursor: "pointer",
              ...
              >
                {showAnagraficaFirme ? "Chiudi gestione" : "Gestisci nominativi"}
              </button>
            </div>
            ...
            {showAnagraficaFirme ? (
              ...
              >
                <div style={{ display: "grid", gap: 6 }}>
                  {getAllSignatureEntries().map((entry) => {
                    const key = signatureKey(entry);
                    const personalizzato = isCustomSignatureEntry(entry);
                    ...
                    return (
                      <div
                        ...
                        style={{
                          display: "grid",
                          gridTemplateColumns: "1fr 150px 110px 90px",
                          alignItems: "center",
                          gap: 8,
                          padding: 6,
                          border: "1px solid #e2e8f0",
                          borderRadius: 8,
                          background: personalizzato ? "#f0fdf4" : "#f8fafc",
                        }}
                      >
                        <span>
                          <strong>{entry.name}</strong>
                          {personalizzato ? (
                            <span style={{ color: "#15803d", fontSize: 11, marginLeft: 6 }}>aggiunto</span>
                          ) : null}
                          <br />
                          <span style={{ color: "#64748b", fontSize: 11 }}>{entry.discipline || "—"}</span>
                        </span>
                        <span style={{ color: "#475569", fontSize: 12 }}>{entry.role}</span>
                        {entry.signature ? (
                          <img
                            src={entry.signature}
                            alt={`Firma ${entry.name}`}
                            style={{ maxWidth: 100, maxHeight: 30, objectFit: "contain" }}
                          />
                        ) : (
                          <span style={{ color: "#94a3b8", fontSize: 11 }}>firma assente</span>
                        )}
                        <button
                          type="button"
                          onClick={() => rimuoviNominativo(entry)}
                          style={{
                ...
                >
                  <div>
                    ...
                    <input
                      style={inputStyle}
                      value={nuovoNominativo.name}
                      onChange={(e) => setNuovoNominativo((prev) => ({ ...prev, name: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      style={inputStyle}
                      value={nuovoNominativo.discipline}
                      onChange={(e) => setNuovoNominativo((prev) => ({ ...prev, discipline: e.target.value }))}
                      placeholder="Es. Progetto Strutturale"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      style={inputStyle}
                      value={nuovoNominativo.role}
                      onChange={(e) => setNuovoNominativo((prev) => ({ ...prev, role: e.target.value }))}
                    >
                      <option value="Ispettore Tecnico">Ispettore Tecnico</option>
                      <option value="Responsabile Tecnico">Responsabile Tecnico</option>
                      <option value="Coordinamento">Coordinamento</option>
                    </select>
                  </div>
                  <div>
                    <label style={labelStyle}>Firma (.png, .jpg)</label>
                    <input
                      style={inputStyle}
                      type="file"
                      accept="image/png,image/jpeg"
                      onChange={(e) => caricaFirmaNuovoNominativo(e.target.files?.[0])}
                    />
                    {nuovoNominativo.signature ? (
                      <img
                        src={nuovoNominativo.signature}
                        alt="Anteprima firma"
                        style={{ maxWidth: 140, maxHeight: 40, objectFit: "contain", marginTop: 6 }}
                      />
                    ) : null}
                  </div>
                  <div style={{ gridColumn: "1 / -1", display: "flex", gap: 8 }}>
                    ...
                    <button
                      type="button"
                      onClick={ripristinaAnagraficaFirme}
                      style={{
                        border: "1px solid #cbd5e1",
                        background: "white",
                        color: "#334155",
                    ...
                    >
                      Ripristina elenco originale
                    </button>
                  </div>
                  <div style={{ gridColumn: "1 / -1", fontSize: 11, color: "#64748b" }}>
                    Aggiunte e rimozioni valgono su questo browser e restano salvate fra una sessione e l'altra.
                  </div>
                </div>
              </div>
            ) : null}
