...
Pasted text.txt
Documento
il fil e ha il seguente percorso : app/api/parse/route.ts

Dettaglio selezione (6)Export


IT22079 ASPI AQ ToDo 20260615173117Todos


T1266 0000 PE DG GEN 00000 00000 E GEN 2001 00 Ispettori REVTable 10


Ti riporto di seguito i Todo esportati per la commessa di maria grazia, il dettaglio estratto dalla piattaforma e il file degli elaborati ufficiali mi dici dove è l'errore

IT22079 ASPI AQ ToDo 20260616061200Todos


Da questo file quanti elababorati unici conti

può essere che il totale dei rilievi siano il totale degli elaborati


Pasted text.txt
Documento
components/AppProgettiUpload.tsx

ora sembra corretto. come faccio a capire se è corretto questo dato anche per altri casi

IT25063 ROMA AQ ToDo 20260616065724Todos


Quanti elaborati unici ha questo progetto


IT22079AR-039-SP-0012-0_Verifiche Preliminari.docx
Documento
le verifiche preliminari non li abbiamo ancora fatte, perchè non sono delle verifiche sul progetto ma delle verifiche sugli elaborati che devono esser presenti secondo tipologia di progetto pfte, definitivo, esecutivo. Ti allego una scheda eseguita


Presentation 1.bcf
File
ho redatto una presentazione in bcf 2.1 con solibri ma nella piattaforma non la vedo nè come dati nè come immagini.  Ti carico il .bcf e l'immagine della piattaforma nexcommon


Pasted text.txt
Documento
ti allego il file esegui i controlli e restituiscimi il file corretto e pronto da incollare

1b0d3918-4aea-49df-bc3b-0ed5434f25d2.png
56ff3f7e-78d5-44e4-a2e4-e85c1e957395.png
IT25085AR AQ Un ToDo 20260616130359Todos


Non esce nella piattaforma tipologia rilievo quando lo eseguo in solibri il bcf. nei todo è la colonna del Tags. come posso far uscire NC oppure OSS etc..?

Non modificherei la visualizzazione del pdf ma inserirei le immagini alla fine della tabella dopo la sintesi facendo un paragrafo Immagini NC/OSS con i relativi link alle NC o OSS


Pasted text.txt
Documento
restituiscimi il file corretto e pronto da sostituire 


ITS QUALITA'260616141722+0000.bcfzip
File
In questo .bcf ho allegato un'immagine ma non viene riprodotta come avviene per l'immagine di solibri

Mi piace molto come evoluzione, ma per procedere mi serve il file aggiornato che stai utilizzando adesso.

Dopo le ultime modifiche abbiamo già toccato:

supporto .bcf Solibri;

immagini BCF nei PDF;

gestione snapshot;

conteggio elaborati unici;

gestione Label NC/OSS.

Per inserire correttamente il raggruppamento per elaborato devo lavorare sull'ultima versione reale di:

components/AppProgettiUpload.tsx
oppure sul file che contiene la funzione:

exportDetailPdf(...)
Per evitare di sovrascrivere modifiche già fatte, carica il file aggiornato e ti restituisco una versione pronta da incollare con:

✅ Ordinamento per elaborato nel PDF
...
PIANO SICUREZZA
NC-02
✅ Pagina iniziale riepilogo:

ELABORATI CON RILIEVI

Relazione Generale ........ 5
Piano Sicurezza ........... 2
Capitolato ................ 1
✅ Sezione finale IMMAGINI NC/OSS invariata

✅ Dashboard invariata

✅ Excel invariato

In questo modo il PDF diventa molto più leggibile per progettisti, PM e Stazione Appaltante senza alterare il resto della piattaforma.
...
Ho analizzato il file che hai caricato. 


La modifica che vuoi fare (ordinare il PDF per elaborato) va fatta dentro la funzione:

function exportDetailPdf(rows: any[], title = "", headerData: PdfHeaderData = {})
Attualmente il PDF viene costruito con:

const tableRows = rows.map((r: any, i: number) => {
quindi segue semplicemente l'ordine dei record presenti nella dashboard.

Quello che ti consiglio
Non limitarti a fare un semplice .sort().

Fai invece un vero raggruppamento per elaborato, perché è molto più leggibile.
...
NC-06
Vantaggi
il progettista apre il PDF e vede subito tutti i rilievi dello stesso elaborato;

coincide con il modo in cui vengono corrette le tavole;

è esattamente il suggerimento di Maria Grazia.

Prima di modificare il codice
Ti consiglio una scelta progettuale:

Opzione A (consigliata)
Lasciare la tabella principale invariata.

Aggiungere una nuova sezione dopo il riepilogo elaborati:

RILIEVI ORDINATI PER ELABORATO
Così hai:

tabella generale (come oggi)

riepilogo elaborati

rilievi raggruppati per elaborato

immagini NC/OSS

Non rompi nulla di ciò che già funziona.

Opzione B
Sostituire completamente la tabella principale.

Io non lo farei perché rischi di scontentare chi ormai si è abituato all'attuale layout.

La mia raccomandazione è:

✅ Aggiungere una nuova sezione "Rilievi ordinati per elaborato"

✅ Mantenere invariata tutta la struttura attuale del PDF

✅ Lasciare la dashboard invariata

In questo modo introduci una funzionalità nuova senza toccare nulla di ciò che oggi sta funzionando correttamente.



Pasted text.txt
Documento
ti riporto il file esegui le modifiche senza alterare quello che già c'è e restituiscimi il file completo e pronto da incollare

Aggiorno solo l’esportazione PDF: mantengo la tabella principale, il riepilogo elaborati e la sezione immagini già esistenti; aggiungo una nuova sezione RILIEVI ORDINATI PER ELABORATO dopo il riepilogo e prima delle immagini.

Ragionato per 6s
Fatto. Ho mantenuto invariato ciò che già c’era e ho aggiunto nel PDF una nuova sezione:

RILIEVI ORDINATI PER ELABORATO
La sezione viene inserita dopo il riepilogo elaborati e prima di IMMAGINI NC/OSS. La dashboard, l’Excel, la tabella principale del PDF e la sezione immagini restano invariati. 
...
Scarica AppProgettiUpload_rilievi_per_elaborato_pdf.tsx
...
if (lower.startsWith("data/jpeg") || lower.startsWith("data/jpg")) return "JPEG";
if (lower.startsWith("data/webp")) return "WEBP";
return "PNG";
}
...
<div style={{ fontSize: 13, color: "#64748b" }}>{title}
<div style={{ fontSize: 34, fontWeight: 800, color: colorValue || "#0f172a" }}>
{value}

{subtitle && <div style={{ fontSize: 12, color: "#64748b" }}>{subtitle}}
...
<h3 style={{ marginTop: 0 }}>File importati
...
return <span style={{ color: "#94a3b8" }}>{emptyText};
}
...
{c.date && <div style={{ color: "#64748b", fontSize: 12 }}>{c.date}}
<div style={{ marginTop: 4 }}>{c.comment || ""}
...
<div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
<h2 style={{ marginTop: 0 }}>Dettaglio selezione: {title}
