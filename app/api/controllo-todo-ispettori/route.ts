...
function hasAnyValidDiscipline(value: string, disciplineAmmesse: Set<string>) {
  ...
  // Regola richiesta: se una delle discipline è "Documenti generali",
  // l'intera cella è valida anche se contiene una seconda disciplina.
...
function isDescrizioneGenerale(value: string) {
  ...
  // Se non contiene un codice elaborato PV..., va considerato testo descrittivo valido.
  // Esempio valido: "Manca un elaborato che dia evidenza..."
...
function findBcfTopic(
  ...
  topics.forEach((topic) => {
    ...
    // Fondamentale: se il BCF contiene IT22-92 deve prevalere su qualsiasi altro TR-92.
    score += textScore(haystack, labelIT, 1000);

    // Matching codice elaborato.
    score += textScore(haystack, comparableCode, 800);

    // Matching TR solo come criterio secondario, perché i TR possono ripetersi tra discipline.
    if (normalizedTr && normalizeCode(topic.tr) === normalizedTr) score += 200;
    else score += textScore(haystack, normalizedTr, 120);

    // Matching disciplina e testo del ToDo per evitare che TR-92 Ambiente prenda commenti Economico.
    score += wordOverlapScore(haystack, disciplina, 20, 100);
    score += wordOverlapScore(haystack, title, 10, 120);
    score += wordOverlapScore(haystack, description, 4, 120);
  ...
  // Soglia minima: evita accoppiamenti casuali tra discipline diverse.
  return bestScore >= 200 ? bestTopic : null;
}
...
function buildChecks(todoRows: any[][], reportCodes: Map<string, string>, disciplineAmmesse: Set<string>, bcfTopics: BcfTopic[]) {
  return todoRows.slice(1).map((row, index) => {
    ...
    const disciplinaOk =
      disciplinaPresente && hasAnyValidDiscipline(disciplina, disciplineAmmesse);
    ...
    // Errori veri: solo campi realmente bloccanti.
    // Codici non trovati, BCF mancanti, discipline non perfettamente allineate e tag mancanti sono warning.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const bcfFiles = formData.getAll("bcf").filter((file): file is File => file instanceof File && file.size > 0);
